...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    let cachedOverrides: Record<string, any> = {};
      ...
      if (val && typeof val === "object" && val.name && !deletedIdsSet.has(key)) {
        if (!initialList.some(i => i.id === key)) {
          initialList.push({
            ...
            recommendedSetsReps: "3-4 Sets x 10-12 Reps",
            ...val
          } as Exercise);
        }
      }
          ...
          Object.keys(mergedOverrides).forEach(key => {
            ...
            if (val && typeof val === "object" && val.name && !activeDeletedIds.has(key)) {
              if (!baseList.some(b => b.id === key)) {
                customMap.set(key, {
                  id: key,
                  category: val.category || "Gym Workouts",
                  difficulty: val.difficulty || "Intermediate",
                  muscleGroups: val.muscleGroups || ["Chest"],
                  equipment: val.equipment || ["Dumbbells"],
...
`
  ...
  const adminTogglePremium = async (exerciseId: string) => {
    let nextPremium = false;
    const updatedExercises = exercises.map(ex => {
      if (ex.id === exerciseId) {
        nextPremium = !ex.isPremium;
    ...
    try {
      ...
      safeSetItem("fit_custom_exercise_overrides", JSON.stringify(overrides));
    } catch {}
    ...
    try {
      ...
      await fetch("/api/exercises/update", {
        method: "POST",
        headers,
        body: JSON.stringify({ exerciseId, updates: { isPremium: nextPremium } })
      });
    } catch (e) {}
    ...
    if (!isMockFirebase) {
      try {
        await setDoc(doc(db, "exercises", exerciseId), { id: exerciseId, isPremium: nextPremium }, { merge: true });
      } catch (e) {}
    }
  ...
  const uploadExerciseMedia = async (exerciseId: string, mediaUrl: string | null, mediaType?: "image" | "video") => {
    ...
    try {
      ...
      if (finalMediaUrl) {
        ...
        if (currentOverrides[exerciseId]) {
          delete currentOverrides[exerciseId].customMediaUrl;
          delete currentOverrides[exerciseId].customMediaType;
        }
      }
  ...
  const editExercise = async (exerciseId: string, updates: Partial<Exercise>): Promise<boolean> => {
    try {
      ...
      // Cache override permanently in localStorage
      try {
        ...
        console.warn("Could not cache exercise override locally:", oErr);
      }
  ...
  const deleteWorkout = async (exerciseId: string): Promise<boolean> => {
    ...
    // 2. Persist deleted ID locally in fit_deleted_exercises and clear override
    try {
