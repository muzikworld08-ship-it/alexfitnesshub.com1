...
import { recordWorkoutCompletion } from "./streakManager";
...
export async function recordDailyWorkoutCompletionAsync(options: {
  ...
  try {
    ...
    // Authoritative Consistency Streak log entry
    recordWorkoutCompletion({
      programId,
      dayNumber: completedDay,
      completedAt: now,
      completedDateStr: localState.completedDateStr,
      source: "program"
    });
  } catch (e) {
...
export function recordDailyWorkoutCompletion(programId: string, completedDay: number): ProgramWaitState {
  ...
  try {
    ...
    recordWorkoutCompletion({
      ...
      dayNumber: completedDay,
      completedAt: now,
      completedDateStr: state.completedDateStr,
      source: "program"
    });
  } catch (e) {
